    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_forget_modify:
                ...
                new PayUtil(com.tck.daole.activity.WaterChargeActivity.this,httpInterface).ApliyPay(no,"",account,itemId);
        ...
        if (NetUtil.isNetWorking(this)) {
            ...
            ThreadPoolManager.getInstance().getNetThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    httpInterface.w_e_g1_charge(city,"c2670",new MApiResultCallback() {
                        @Override
...
//                                        String message=jsonObject.optString("message");
                                        ...
                                        if (status==1 && StringUtil.isSpace(sub_code)){
                                            ...
                                            if (items!=null&&items.item.size()>0){
                                                ...
                                                WaterChargeActivity.this.items.setShowEmptyText(true);
                                            }
                                        }else {
                                            toast("充值失败，请重试");
                                        }
